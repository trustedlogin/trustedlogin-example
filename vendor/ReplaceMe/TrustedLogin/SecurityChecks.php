...
final class SecurityChecks {
	...
	/**
	 * @var string ReplaceMe\TrustedLogin endpoint to notify brute-force activity
	 */
	const BRUTE_FORCE_ENDPOINT = 'report-brute-force';

	/**
	 * @var string ReplaceMe\TrustedLogin endpoint to verify valid support activity
	 */
	...
	public function verify( $passed_user_identifier = '' ) {
		...
		if ( $this->in_lockdown() ){
			...
			return new WP_Error( 'in_lockdown', __( 'ReplaceMe\TrustedLogin temporarily disabled.' , 'trustedlogin') );
		}
	...
	/**
	 * Makes double-y sure the ReplaceMe\TrustedLogin Server approves this support-agent login.
	 *
	 * This function sends server variables to the ReplaceMe\TrustedLogin server to help prevent a number of attack vectors.
	 ...
	 *
	 * @return true|WP_Error True: the ReplaceMe\TrustedLogin service was reached and the login remains valid. WP_Error: The service wasn't reachable or the service responded that the secret ID wasn't valid.
	 */
	private function check_approved_identifier( $secret_id ) {
		 ...
		 *
		 * No site user/visitor/admin data is sent back to ReplaceMe\TrustedLogin server.
		 */
		$body = array(
			'timestamp'  => time(),
	...
	/**
	 * Notifies the ReplaceMe\TrustedLogin server that a site may be under a possible brute-force attack.
	 *
	 * @since  1.0.0
	 *
	...
	/**
	 * Locks down the site to new access by ReplaceMe\TrustedLogin identifiers, reports lockdown to ReplaceMe\TrustedLogin
	 */
	private function do_lockdown() {
		...
		if ( is_wp_error( $notified ) ){
			$this->logging->log( sprintf( 'Could not notify ReplaceMe\TrustedLogin (%s)', $notified->get_error_message() ), __METHOD__, 'error' );
		}
	...
	/**
	 * Checks if ReplaceMe\TrustedLogin is currently in lockdown
	 *
