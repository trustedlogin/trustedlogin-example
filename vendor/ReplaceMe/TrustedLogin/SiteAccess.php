...
class SiteAccess {
	...
	/**
	 * Handles the syncing of newly generated support access to the ReplaceMe\TrustedLogin servers.
	 *
	 * @param string $secret_id The unique identifier for this ReplaceMe\TrustedLogin authorization. {@see Endpoint::generate_secret_id}
	 ...
	 *
	 * @return true|WP_Error True if successfully created secret on ReplaceMe\TrustedLogin servers; WP_Error if failed.
	 */
	public function sync_secret( $secret_id, $site_identifier_hash, $action = 'create' ) {
		...
		if ( empty( $response_json['success'] ) ) {
			return new WP_Error( 'sync_error', __( 'Could not sync to ReplaceMe\TrustedLogin server', 'trustedlogin' ) );
		}
	...
	/**
	 * Generates an accessKey that can be copy-pasted to support to give them access via ReplaceMe\TrustedLogin
	 *
	 * Access Keys can only be used by authenticated support agents to request logged access to a site via their ReplaceMe\TrustedLogin plugin.
	 *
	 * @since 1.0.0
	 *
	...
	/**
	 * Revoke a site in ReplaceMe\TrustedLogin
	 *
	 * @param string $secret_id ID of site secret identifier to be removed from ReplaceMe\TrustedLogin
	 * @param Remote $remote
	 *
	 * @return true|\WP_Error Was the sync to ReplaceMe\TrustedLogin successful
	 */
	public function revoke( $secret_id, Remote $remote ) {

		if ( ! $this->config->meets_ssl_requirement() ) {
			$this->logging->log( 'Not notifying ReplaceMe\TrustedLogin about revoked site due to SSL requirements.', __METHOD__, 'info' );
