 ...
 *
 * 0. If you haven't already, sign up for a ReplaceMe\TrustedLogin account {@see https://www.trustedlogin.com}
...
final class Client {
	...
	/**
	 * ReplaceMe\TrustedLogin constructor.
	 *
	...
	public function __construct( Config $config, $init = true ) {
		...
		if ( ! $should_initialize ) {
			throw new \Exception( 'ReplaceMe\TrustedLogin was prevented from loading by constants defined on the site.', 403 );
		}
	...
	public function get_access_key() {
		...
		if ( ! self::$valid_config ) {
			return new WP_Error( 'invalid_configuration', 'ReplaceMe\TrustedLogin has not been properly configured or instantiated.', array( 'error_code' => 424 ) );
		}
	...
	/**
	 * This creates a ReplaceMe\TrustedLogin user ✨
	 *
	...
	public function grant_access() {
		...
		if ( ! $this->config->meets_ssl_requirement() ) {
			return new WP_Error( 'fails_ssl_requirement', esc_html__( 'ReplaceMe\TrustedLogin requires a secure connection using HTTPS.', 'trustedlogin' ) );
		}
	...
	private function extend_access( $user_id ) {
		...
		try {
			...
			$this->logging->log( 'There was an error updating ReplaceMe\TrustedLogin servers.', __METHOD__, 'error', $e );
