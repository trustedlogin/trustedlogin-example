...
final class Admin {
	...
	/**
	 * Is this a login screen and should ReplaceMe\TrustedLogin override the login screen for the current namespace?
	 *
	 * @return bool
	 */
	...
	/**
	 * If the current request is a valid login screen override, print the ReplaceMe\TrustedLogin request screen.
	 *
	 * @return void
	 */
...
';
	...
	/**
	 * Outputs the ReplaceMe\TrustedLogin authorization screen
	 *
	 * @since 1.0.0
	 *
	 * @return void
	...
	/**
	 * Output the ReplaceMe\TrustedLogin Button and required scripts
	 *
	 * @since 1.0.0
	 *
	...
	/**
	 * Generates HTML for a ReplaceMe\TrustedLogin Grant Access button
	 *
	 * @param array $atts {
	 ...
	 *   @type bool   $powered_by Whether to display the ReplaceMe\TrustedLogin badge on the button. Default: true
	...
	public function translations() {
		...
		$query_args = apply_filters( 'trustedlogin/' . $this->config->ns() . '/support_url/query_args', array(
				'message' => __( 'Could not create ReplaceMe\TrustedLogin access.', 'trustedlogin' ),
		...
		$translations = array(
			...
			'status'  => array(
				...
				'accesskey'          => array(
					'title'       => esc_html__( 'ReplaceMe\TrustedLogin Key Created', 'trustedlogin' ),
					'content'     => sprintf(
						// translators: %1$s is the vendor title
						__( 'Share this ReplaceMe\TrustedLogin Key with %1$s to give them secure access:', 'trustedlogin' ),
						$vendor_title
					),
					'revoke_link' => esc_url( add_query_arg( array( Endpoint::REVOKE_SUPPORT_QUERY_PARAM => $this->config->ns() ), admin_url() ) ),
				),
				'error404'           => array(
					'title'   => esc_html__( 'The ReplaceMe\TrustedLogin vendor could not be found.', 'trustedlogin' ),
					'content' => '',
				),
				'error409'           => array(
					'title'   => sprintf(
...
EOD;
			$access_key_output   = sprintf(
				...
				// translators: %s is the display name of the ReplaceMe\TrustedLogin support user.
